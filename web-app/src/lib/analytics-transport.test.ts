...
describe('analytics transport hardening', () => {
	...
	it('does not interpolate raw relay URLs into invalid URL copy', () => {
		for (const locale of readdirSync('web-app/src/locales')) {
			const commonJson = JSON.parse(
				readFileSync(`web-app/src/locales/${locale}/common.json`, 'utf8')
			) as {
				settings?: { customRelays?: { invalidUrl?: string } }
			}
			const invalidUrl = commonJson.settings?.customRelays?.invalidUrl

			assert.equal(
				invalidUrl?.includes('{{url}}') ?? false,
				false,
				`${locale} must not echo raw invalid relay URLs`
			)
		}
	})
})
